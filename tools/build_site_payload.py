...
TRANSLATIONS["ja"].update(
    {
        ...
        "category.drop-lab.title": "Drop Lab",
        "category.drop-lab.description": "宝箱、ドロップテーブル、対象アイテム、出現ステージを横断して入手確率を確認します。",
        ...
        "drop.title": "Drop Lab",
        "drop.subtitle": "宝箱から中身へ、アイテムから入手先へ。ドロップテーブルとステージ報酬を行き来しながら周回先を決めます。",
        "drop.search": "ドロップ検索",
        "drop.placeholder": "箱、アイテム、ステージ、DropKey、ID",
        "drop.viewMode": "表示対象",
        "drop.all": "すべて",
        "drop.chests": "宝箱",
        "drop.items": "アイテム",
        ...
        "drop.contents": "中身",
        "drop.sources": "出現ステージ",
        "drop.bestSources": "主な入手先",
        "drop.groupItems": "候補アイテム",
        "drop.dropKey": "DropKey",
        "drop.openChest": "箱を開く",
        "drop.openItem": "アイテムを開く",
        "drop.ownedHint": "セーブ所持",
        "drop.noSelection": "左の候補から宝箱またはアイテムを選択してください。",
        "drop.rateHint": "箱の出現率と中身の重みを分けて表示します。必要に応じてステージ詳細でWaveと敵構成も確認できます。",
...
TRANSLATIONS["en"].update(
    {
        ...
        "category.drop-lab.title": "Drop Lab",
        "category.drop-lab.description": "Cross-check chests, drop tables, target items, and source stages to inspect acquisition rates.",
        "category.progress-planner.title": "Progress Planner",
        ...
        "drop.title": "Drop Lab",
        "drop.subtitle": "Move from chest to contents or from item to source stages, then choose where to farm.",
        "drop.search": "Drop search",
        "drop.placeholder": "Chest, item, stage, DropKey, or ID",
        "drop.viewMode": "Target",
        "drop.all": "All",
        "drop.chests": "Chests",
        "drop.items": "Items",
        "drop.results": "Candidates",
        "drop.contents": "Contents",
        "drop.sources": "Source stages",
        "drop.bestSources": "Best sources",
        "drop.groupItems": "Candidate items",
        "drop.dropKey": "DropKey",
        "drop.openChest": "Open chest",
        "drop.openItem": "Open item",
        "drop.ownedHint": "Owned in save",
        "drop.noSelection": "Pick a chest or item from the candidate list.",
        "drop.rateHint": "Chest appearance rates and content weights are shown separately. Open stage details when wave and monster context matters.",
        "plan.title": "Progress Planner",
...
def build_manifest(categories: dict[str, dict[str, Any]], entry_map: dict[str, list[dict[str, Any]]], assets: dict[str, str]) -> dict[str, Any]:
    ...
    nav_groups = [
        {"id": "nav.tools", "items": ["my-save", "market", "farm-planner", "drop-lab", "progress-planner", "lab-status"]},
...
def main() -> None:
    ...
    categories["drop-lab"] = category_definition("drop-lab", len(load_data("drops")) + len(load_table("ItemInfoData")) + len(load_table("StageInfoData")), asset(assets, "Item_910011") or asset(assets, "MenuButton_Portal_Active"), "cards", "nav.tools")
